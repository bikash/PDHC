...
/**********************************************************
 * The checker NameNode is a helper to the primary NameNode.
 * The Secondary is responsible for supporting periodic checkpoints 
 * of the HDFS metadata. The current design allows only one Secondary
 * NameNode per HDFs cluster.
 *
 * The checker NameNode is a daemon that periodically wakes
...
public class Checker implements Runnable,
  ...
  public static final Log LOG = 
    LogFactory.getLog(Checker.class.getName());

private static final String CheckpointFaultInjector = null;
   ...
   */
  public Checker(Configuration conf)  throws IOException {
    this(conf, new CommandLineOpts());
  }
  
  public Checker(Configuration conf,
      CommandLineOpts commandLineOpts) throws IOException {
    try {
      String nsId = DFSUtil.getSecondaryNameServiceId(conf);
      if (HAUtil.isHAEnabled(conf, nsId)) {
        throw new IOException(
            "Cannot use CheckerNode in an HA cluster." +
  ...
  public static InetSocketAddress getHttpAddress(Configuration conf) {
    return NetUtils.createSocketAddr(conf.get(
    		ConfigKey.DFS_NAMENODE_CHECKER_HTTP_ADDRESS_KEY,
    		ConfigKey.DFS_NAMENODE_CHECKER_HTTP_ADDRESS_DEFAULT));
  }
  
  /**
   * Initialize CheckerNameNode.
   */
  private void initialize(final Configuration conf,
    ...
    DefaultMetricsSystem.initialize("CheckerNode");
    JvmMetrics.create("CheckerNode",
    ...
    final String httpsAddrString = conf.get(
    		ConfigKey.DFS_NAMENODE_CHECKER_HTTPS_ADDRESS_KEY,
    		ConfigKey.DFS_NAMENODE_CHECKER_HTTPS_ADDRESS_DEFAULT);
    ...
    infoServer.setAttribute("checker.name.node", this);
    ...
    if (policy.isHttpEnabled()) {
      ...
      conf.set(ConfigKey.DFS_NAMENODE_CHECKER_HTTP_ADDRESS_KEY,
          NetUtils.getHostPortString(httpAddress));
    }

    if (policy.isHttpsEnabled()) {
      InetSocketAddress httpsAddress = infoServer.getConnectorAddress(connIdx);
      conf.set(ConfigKey.DFS_NAMENODE_CHECKER_HTTPS_ADDRESS_KEY,
